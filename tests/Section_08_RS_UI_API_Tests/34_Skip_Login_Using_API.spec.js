...
test('Section_08_RS_UI_API_Tests - Practice Client App - Skip Login using API call - Assert Tab: Home', async ({page} )=> {
    const tab_Home = page.locator("[routerlink='/dashboard/']");
    console.log('Assertions for tab: Home')
    await expect(tab_Home).toBeVisible();   
});

test('Section_08_RS_UI_API_Tests - Practice Client App - Skip Login using API call - Assert Tab: Sign Out', async ({page} )=> {
